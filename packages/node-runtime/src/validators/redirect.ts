...
import type { VerifyOptions, RedirectTrace, RedirectAnomalyKind } from '@safe-link-checker/core';
...
export async function traceRedirects(
  ...
  for (let hop = 0; hop < maxRedirects; hop++) {
    ...
    if (current.startsWith('https://') && nextUrl.startsWith('http://')) {
      ...
      break; // Abort redirect tracing immediately for security
    }
...
function headRequest(urlStr: string, timeoutMs: number, signal?: AbortSignal): Promise<HeadResult | null> {
  return new Promise((resolve) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      // Security: Abort on non-HTTP schemas (ftp://, file://, data://, blob://)
      return settle(null);
    }

    // SSRF / DNS Protection (blocks 127.0.0.1, 10.x, etc.)
    // Explicitly check localhost before even resolving
    if (parsed.hostname === 'localhost' || parsed.hostname === '127.0.0.1' || parsed.hostname === '0.0.0.0' || parsed.hostname === '::1') {
